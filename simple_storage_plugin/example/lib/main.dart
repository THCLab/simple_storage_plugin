                ...
                RawMaterialButton(
                  onPressed: () async{
                    if(writeDataController.text.isNotEmpty && writeKeyController.text.isNotEmpty){
                      var result = await SimpleStoragePlugin.writeData(writeKeyController.text, writeDataController.text);
                      if(result == true){
                        setState(() {
                          writeResult = 'Success!';
                        });
                ...
                RawMaterialButton(
                  onPressed: () async{
                    if(readKeyController.text.isNotEmpty){
                      var result = await SimpleStoragePlugin.readData(readKeyController.text);
                      if(result != false){
                        setState(() {
                          readResult = result;
                        });
                ...
                RawMaterialButton(
                  onPressed: () async{
                    if(deleteKeyController.text.isNotEmpty){
                      var result = await SimpleStoragePlugin.deleteData(deleteKeyController.text);
                      if(result == true){
                        setState(() {
                ...
                RawMaterialButton(
                  onPressed: () async{
                    if(editDataController.text.isNotEmpty && editKeyController.text.isNotEmpty){
                      var result = await SimpleStoragePlugin.editData(editKeyController.text, editDataController.text);
                      if(result == true){
                        setState(() {
                          editResult = 'Success!';
                        });
